...
const AdminDonation = () => {
  // 1. STATES
  const [view, setView] = useState('muni'); // muni, brgy, list, details
  ...
  const fetchAdminDonations = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:3000/api/v1/donate', config);
  ...
  // 3. ACTION HANDLER (Verification)
  const handleVerifyAction = async (id, status, adminNote) => {
    try {
      const userInfo = JSON.parse(localStorage.getItem('userInfo'));
      const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
      
      await axios.put(`http://localhost:3000/api/v1/donate/${id}`, { 
        status, 
      ...
      setModal({ 
        ...
        title: status === 'received' ? 'Donation Verified' : 'Donation Cancelled', 
        message: status === 'received' ? 'Official receipt has been deployed.' : 'The record has been rejected.' 
      });
  ...
  return (
    <Sidebar>
      <div className="max-w-7xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
        ...
        {/* STAT CARDS (Visible only on Muni view for overview) */}
        {/* STAT CARDS (Visible only on Muni view for overview) */}
        {view === 'muni' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* 2. BAGONG CARD: Environmental Monitoring Fund */}
            <StatCard 
            ...
            {/* 3. BAGONG CARD: Disaster Relief Fund */}
            <StatCard 
        ...
        <div className="min-h-[500px]">
                  ...
                  <tbody className="divide-y divide-gray-50">
                     {data?.donations?.filter(d => d.municipality === selection.municipality && d.barangay === selection.barangay).map(d => (
                       <tr key={d._id} className="hover:bg-green-50/40 transition-all">
                          ...
                          <td className="p-8">
                            <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase ${d.status === 'received' ? 'bg-green-100 text-green-700 shadow-sm' : 'bg-yellow-100 text-yellow-700'}`}>
                              {d.status}
                            </span>
                          </td>
          ...
          {/* D. FULL DETAIL VIEW (THE FOLDER) */}
          {view === 'details' && selectedDonation && (
            <div className="animate-in slide-in-from-right-10 duration-500 pb-10">
               <button onClick={() => setView('list')} className="flex items-center gap-2 text-gray-400 font-black text-[10px] uppercase mb-8 hover:text-[#166534] tracking-widest transition-all"><ArrowLeft size={14}/> Close Registry File</button>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  ...
                  <div className="bg-white p-12 rounded-[.5rem] shadow-2xl border-4 border-green-50 space-y-8 relative overflow-hidden h-fit">
                      ...
                      <div className="pt-6 space-y-4">
                         ...
                         <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Type a thank you message..." className="w-full p-6 bg-gray-50 border border-gray-100 rounded-[.5rem] outline-none h-24 focus:ring-2 ring-green-100 text-sm font-medium" />
                      </div>

                      {selectedDonation.status === 'pending' ? (
                        <div className="flex gap-4">
                           <button onClick={() => handleVerifyAction(selectedDonation._id, 'received', note)} className="flex-[3] bg-[#166534] text-white py-5 rounded-2xl font-black text-lg shadow-xl shadow-green-100 hover:scale-[1.02] active:scale-95 transition-all">VERIFY & DEPLOY</button>
                           <button onClick={() => handleVerifyAction(selectedDonation._id, 'rejected', 'Invalid Reference Number')} className="flex-1 bg-red-50 text-red-600 rounded-2xl font-black text-xs hover:bg-red-500 hover:text-white transition-all uppercase">Reject</button>
                        </div>
                      ) : (
                        <div className="p-4 bg-[#166534] text-[#bef264] rounded-[.5rem] text-center font-black uppercase tracking-widest flex items-center justify-center gap-3">
                           <ShieldCheck /> Receipt Successfully Issued
                        </div>
                      )}
                  </div>
               </div>
