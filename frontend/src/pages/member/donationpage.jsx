...
const DonatePage = () => {
  ...
  // 2. FETCH DATA (Wrapped in useCallback to fix dependency error)
      ...
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in pb-20 px-4">
        ...
        {step === 'form' && (
          <form onSubmit={(e) => { e.preventDefault(); setStep('preview'); }} className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-4">
            <div className="lg:col-span-2 bg-white p-10 rounded-2xl border border-gray-100 shadow-sm space-y-8 h-fit">
            ...
            <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6 h-fit text-center">
                ...
                <label className="cursor-pointer group block">
                  <div className="bg-gray-50 p-10 rounded-xl border-2 border-dashed border-gray-200 group-hover:border-[#166534] transition-all flex flex-col items-center">
                    <UploadCloud size={40} className="text-gray-300 mb-2 group-hover:text-[#166534]" />
        ...
        {step === 'preview' && (
          <div className="max-w-xl mx-auto bg-white p-12 rounded-[2rem] shadow-2xl border border-gray-100 space-y-8 animate-in zoom-in-95 text-center">
            <h2 className="text-2xl font-black text-gray-900 tracking-tighter uppercase tracking-widest">Confirmation</h2>
            <div className="space-y-4 bg-gray-50 p-8 rounded-2xl text-left border">
              ...
              <div className="flex justify-between items-center"><span className="text-gray-400 font-bold uppercase text-[10px]">Reference</span> <span className="font-mono font-bold text-blue-600">#{formData.referenceNumber}</span></div>
            </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            {!selectedDonation ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-3">
                  ...
                  {myDonations.length === 0 ? (
                    <div className="p-20 text-center bg-white rounded-2xl border-2 border-dashed text-gray-300 font-bold italic uppercase tracking-widest text-xs">No records available</div>
                  ) : myDonations.map(d => (
                    <div key={d._id} onClick={() => setSelectedDonation(d)} className="group bg-white rounded-2xl border border-gray-100 flex items-center cursor-pointer hover:shadow-lg hover:border-green-200 transition-all overflow-hidden">
                      ...
                      <div className="flex-1 flex items-center justify-between p-5">
                        <div className="flex items-center gap-5">
                          ...
                          <div><p className="font-black text-gray-800 text-lg">₱{d.amount}</p><p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{d.category}</p></div>
                        </div>
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                <button onClick={() => setSelectedDonation(null)} className="flex items-center gap-2 text-gray-400 font-black text-[10px] uppercase hover:text-green-700 tracking-widest"><ArrowLeft size={16}/> Back to history</button>
                   ...
                   <div className="p-12 space-y-10">
                      <div className="flex justify-between items-start border-b border-dashed pb-8">
                         <div>
                            ...
                            {selectedDonation.status === 'received' && <p className="text-emerald-600 font-mono text-xs font-bold mt-2">OR#: {selectedDonation.officialReceiptNo}</p>}
                         </div>
                         <div className="flex flex-col items-end gap-3">
                            <StatusBadge status={selectedDonation.status}/>
                            {selectedDonation.status === 'pending' && (
                              <div className="flex gap-2">
                                 <button onClick={() => handleEditInitiate(selectedDonation)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-500 hover:text-white transition-all"><Edit3 size={16}/></button>
                      ...
                      {selectedDonation.status === 'received' && (
                        <div className="p-6 bg-[#166534] text-green-100 rounded-2xl italic text-sm text-center relative mt-6">
                           ...
                           " {selectedDonation.adminNote || "The MaCync Provincial Treasury has officially verified this contribution. Thank you for supporting our environmental monitoring efforts."} "
                        </div>
                      )}
                   </div>
                </div>
